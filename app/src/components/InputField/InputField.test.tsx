...
describe("InputField component unit tests", () => {
  ...
  it("renders the correct label, placeholder, and type", () => {
    render(<InputField id="email" label="Email" type="email" placeholder="Enter your email" />);
  ...
  it("calls the onChange handler when the input value changes", () => {
    ...
    render(<InputField id="email" label="Email" type="email" value={initialValue} onChange={handleChange} />);
...
//   });
  ...
  it("displays the character count when maxLength is set", () => {
    const maxLength = 20;
    render(<InputField id="Username" label="Username" maxLength={maxLength} value="Initial" />);
